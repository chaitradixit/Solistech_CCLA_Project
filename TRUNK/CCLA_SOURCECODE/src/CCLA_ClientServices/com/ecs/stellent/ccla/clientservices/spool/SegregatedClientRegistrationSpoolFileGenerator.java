...
/** Generates a pre-filled Segregated Client Registration Form spool file.
 * 
 ...
 *
 */
public class SegregatedClientRegistrationSpoolFileGenerator extends SpoolFileGenerator {
	...
	private String PAGE_REPORT_ID_PREFIX = "SCCIF";
	
	private static class ReportSections {
		static final String FRONT_COVER 				= "CVR";
		...
		static final String SIGNATURES					= "S6";
	}
	 ...
	 *  
	 */
	private SegregatedClientRegistrationForm regForm;
	...
	public SegregatedClientRegistrationSpoolFileGenerator
	 (SpoolHeader spoolHeader, SegregatedClientRegistrationForm regForm) {
	...
	protected void addReportHeader(String reportId) 
	 throws IOException, DataException {
		...
		if (!reportId.equals(PAGE_REPORT_ID_PREFIX + ReportSections.FRONT_COVER)) {
			addNameValueString
	...
	private void appendFormData()
	 throws DataException, IOException, ServiceException {
		...
		// Front Cover (page 1)
		// ---------------------------------
		// Displays correspondent name, postal address
		addReportHeader(getReportId(ReportSections.FRONT_COVER, null));
		...
		*/
		...
		// ---------------------------------
		// Displays some Org Identifiers and attributes
		...
		addNameValueString("RegWithCharityCommOrOSCR", 
		 SpoolFileUtils.getTickboxValue(charityRegNumber != null));
		...
		Vector<SpoolFileLine> orgAddressLines = SpoolFileGenerator.getGridFormatAddress
		 (orgAddress, "OrgAddressLine", false, false, 21, 4);
		...
		// Org Details (Section 2)
		// ---------------------------------
		// Displays misc org details
		addReportHeader(getReportId(ReportSections.ORG_DETAILS, 2));
		
		addNameValueString("OrgHasOfficeAddress", 
		 SpoolFileUtils.getTickboxValue(orgAddressContact != null));
		...
		Vector<SpoolFileLine> companyAddressLines = 
		 ...
		 (orgCompanyAddress, "CompanyAddressLine", false, false, 21, 4);
		...
		*/
		...
		// First page of section is a cover sheet with a single tick value
		...
		// Determine number of directors to display.
		int numPrintedDirectorOrControllers = 
			Math.max(authPersons.size(), MIN_LISTED_AUTH_PERSONS);

		addNameValueString
		 ("NumberOfDirectors", Integer.toString(numPrintedDirectorOrControllers));
		
		// Print names of directors (up to 4 total)
		for (int i=1; i<=4; i++) {
			...
			addNameValueString("TrusteeDirectorName" + i, 
			 thisAuthPerson == null ? null : thisAuthPerson.getFullName());
		}
		...
		if (mainContact != null && authPersons.contains(mainContact)) {
			...
			Vector<SpoolFileLine> corrAddressLines = 
			 SpoolFileGenerator.getGridFormatAddress
			 (corrAddress, linePrefix + "AddressLine", false, false, 21, 3);
			...
			*/
			...
			addNameValueString(linePrefix + "IsMainContact", "Y");
	...
	private void preparePersonMapping() throws DataException {
		...
		// -0 front cover sheet (not included in page count!)
		...
		EnrolmentAttributeApplied minAuthPersonsAttr = 
		 ...
		 (ApplicableEnrolmentAttribute.Ids.SEG_CLIENT_MIN_AUTH_PERSONS);
		...
		EnrolmentAttributeApplied minSigsAttr = 
		 regForm.getEnrolmentAttributes().get
		 (ApplicableEnrolmentAttribute.Ids.SEG_CLIENT_MIN_SIGNATORIES);
		...
		*/
		...
		// Check that all persons to be printed have complete name data.
		for (Person p : printedPersons) {
			if (StringUtils.stringIsBlank(p.getLastName())) {
				throw new DataException("Person record '" + p.getFullName() +  "' does " +
				 "not have constituent name fields set. Update their record to ensure " +
				 "the separate name fields are set correctly");
			}
		}
	...
	private void addDirectorOrController(Person person, int directorNumber, 
	 int totalDirectorOrControllers) throws DataException, IOException {
		...
		// Add verification field values
		// TODO: should be left blank?
		addNameValueString(linePrefix + "PassportNumber", null);
		addNameValueString(linePrefix + "PassportFullName", null);
