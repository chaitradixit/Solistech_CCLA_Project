...
public class PersonProcessHandler {
	
	protected static final String INITIAL_STATUS = "Pending";
	...
	private PersonProcessHandler() {}
	
	public PersonProcessHandler(Campaign campaign, String userName, FWFacade facade) {
		this.campaign = campaign;
		this.userName = userName;
		this.facade   = facade;
	}
	
	/** Adds a new person process, for the given person ID.
	 *  
	 * @param personId
	 ...
	 * @return the newly-created PersonProcess instance
	 * @throws DataException
	 * @throws ServiceException
	 */
	public PersonProcess addNewProcess(int personId, 
	 int createMethod, boolean exclude) 
	 throws DataException, ServiceException {
		...
		Log.debug(create + this.campaign.getName() + 
		 " campaign for person " + personId);
				
		try {
			facade.beginTransaction();
			
			// Fetch new primary key for this process
			String id = CCLAUtils.getNewKey("Process", facade);
			
			String status = INITIAL_STATUS;
			...
			PersonProcess process = 
			 PersonProcess.add(this.campaign.getCampaignId(), status, 
			 this.userName, personId, curDate, facade);
			...
			String action 		= "Person " + actionVerb + " " + method;
			
			process.addActivity(
			 this.userName, null, processType, action, null, 
			 false, facade);
	 ...
	 * @throws ServiceException 
	 */
	public void applyAction(PersonProcess process, String action, String note) 
	 throws DataException, ServiceException {
		
		Log.debug("Default Person Process Handler: applying action: " + action);
		
		if (action.equals("Exclude")) {
			setExcluded(process, true, note);
			return;
		} else if (action.equals("Enroll")) {
			setExcluded(process, false, note);
			return;
		}
		...
		/*
		ClientServicesService.addOrUpdateActivity(
		 Integer.toString(process.getProcessId()), null, userName, "Status update", null, 
		 "Status changed to " + action, process.getClientId(), 
		 process.getPersonId(), null, facade);
		 */
	...
	protected void setStatus(PersonProcess process, String status) 
	 throws DataException, ServiceException {
	 ...
	 * @throws ServiceException
	 */
	public void setExcluded(PersonProcess process, boolean excluded, String note) 
	 throws DataException, ServiceException {
		...
		if (excluded && !curStatus.equals(EXCLUDED_STATUS)) {
			...
			/*
			ClientServicesService.addOrUpdateActivity(
			 Integer.toString(process.getProcessId()), null, userName, "Exclusion", null, 
			 "Client excluded from campaign", process.getClientId(), 
			 process.getPersonId(), null, facade);
			 */
			...
			setStatus(process, INITIAL_STATUS);
			...
			/*
			ClientServicesService.addOrUpdateActivity(
			 Integer.toString(process.getProcessId()), null, userName, "Enrolment", null, 
			 "Client enrolled to campaign", process.getClientId(), 
			 process.getPersonId(), null, facade);
			 */
		}
	}
	 ...
	 * @return
	 */
	public Vector<String> getAvailableActions(PersonProcess process) {
	 ...
	 * @return
	 */
	public DataResultSet getAvailableActionsResultSet(PersonProcess process) {
	 ...
	 * @return
	 */
	public boolean isValidAction(PersonProcess process, String action) {
	 ...
	 * @throws DataException */
	public void cancelProcess(PersonProcess process) throws DataException {
		
		Log.debug("Deleting process: " + process.getProcessId());
		
		DataBinder binder = new DataBinder();
		binder.putLocal("processId", Integer.toString(process.getProcessId()));
