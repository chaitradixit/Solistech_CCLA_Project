...
function Epoch(name,mode,targetelement,toggleelement,multiselect)
{
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		if (toggleelement) {
			// The toggle element is used to show/hide the calendar
			this.toggle = toggleelement;
			
			// register the calendar with its toggle button and vice-versa
			this.toggle.calendar = this;	
			this.calendar.toggle = this.toggle;
	...
	else
	{
		...
		// Commented this line due to conflict with drop-down linked lists in S&F component
...
// TM: This function prevents more than one active calendar on the 
// screen at one time
function setActiveCalendar(cal) {
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearLower = 2001;
	this.rangeYearUpper = 2037;
	this.minDate = new Date(2001,0,1);
...
/* SL: replacing with something that also works with IE6 and 7 and FF!
	var oNode = element;
	var iTop = 0;
...
{
	setActiveCalendar(this);

	this.calendar.style.display = 'block';
	this.visible = true;
	
	if (this.calendar.toggle) {
		this.calendar.toggle.className="calbutton_sel";
	}
...
Epoch.prototype.hide = function () //PUBLIC: Hides the calendar
{
	...
	if (this.calendar.toggle) {
		this.calendar.toggle.className="calbutton";
	}
